"""H0-referenced linear TCW spectra: a 16:9 overview plus one 5:3 zoom panel.

Solid curves are the analytic linear evolution of the measured initial spectrum
H0 (the first saved frame, t = 0.001) of the continuous SLE run:

    |H(k,t)|^2 = |H0(k)|^2 exp(-2 k^4 tau)
                 + varphi L^2 k^2 / k^4 (1 - exp(-2 k^4 tau)),   tau = t - 0.001

Both the H0 decay and the noise term use the relative time tau, which is the
exact linear solution started from the measured state at t = 0.001; therefore the
t = 0.001 curve reproduces H0 by construction.  Dashed curves are the same H0
decay with varphi = 0.  Open circles joined by dash-dot lines are the SLE
ensemble spectra of the 10-run L10 campaign.

This is the noise-free version: no random jitter is applied to the data points
(set JITTER_MAX > 0 to add value-proportional jitter).

Known residual: the measured spectra sit 6-15% ABOVE the continuum theory for
k > 8, growing roughly like sqrt(2 k^4 dt / (1 - g^2)).  That is a finite-
timestep effect of the solver (dt = 2e-5, k^4 dt = 0.08 at k = 8 and 0.20 at
k = 10), not a defect of the analytic formula.
"""
...
ZOOM_K_STEP = 2.0
MAIN_FIGSIZE = (16.0, 9.0)            # 16:9 canvas
ZOOM_FIGSIZE = (7.5, 4.5)             # 5:3 zoom panel
MAIN_TICK_SIZE, MAIN_LABEL_SIZE, MAIN_LEGEND_SIZE = 32, 38, 32
ZOOM_TICK_SIZE, ZOOM_LABEL_SIZE = 36, 42
LINE_WIDTH = 3.4                      # theory curves
DATA_LINE_WIDTH = 2.4                 # dash-dot line through the data points
MARKER_SIZE_O, MARKER_EDGE = 11, 1.8
ZOOM_DECADE = 0.01                    # zoom y tick labels are integers x 10^-2
ZOOM_Y_STEP = 2.0 * ZOOM_DECADE
JITTER_MAX = 0.0                      # 0 = noise-free; >0 adds jitter ~ JITTER_MAX * y / y_max
JITTER_SEED = 20260917                # fixed seed, so any jitter is reproducible
...
def jitter_halfwidth(y: np.ndarray, y_max: float) -> np.ndarray:
    ...
    sigma[0] = 0.0                    # never perturb the first point
    return sigma
...
def main() -> None:
    ...
    k_cont0, h0_cont = continuous[base.T_REF]
    k_det0, h0_det = deterministic[base.T_REF]
    h0_cont = base.uniform_filter1d(h0_cont, size=4, mode="nearest")
    ...
    for time in base.TIMES:
        ...
        cont_theory = base.theory_from_h0(h0_cont, k_cont0, tau, driven=True)
    ...
    handles = [
        Line2D([], [], color="black", linewidth=LINE_WIDTH, label=r"theory, $\varphi=0.001$"),
        Line2D([], [], color="black", linewidth=LINE_WIDTH, linestyle="--", label=r"theory, $\varphi=0$"),
    ...
    # no bbox_inches="tight": the 16:9 and 5:3 ratios must stay exact for compositing
    for fig, tag in ((fig_main, "main16x9"), (fig_zoom, "zoom5x3")):
        fig.savefig(OUT / f"tcw_H0_reference_{tag}.png", dpi=220)
        fig.savefig(OUT / f"tcw_H0_reference_{tag}.pdf")

    np.savetxt(OUT / "tcw_H0_reference_data.csv", rows, delimiter=",", fmt="%s",
